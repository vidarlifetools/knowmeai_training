...
class UpdateFeatures(DataModule):
        ...
        if type(msg) == UpdateFilesMessage:
            ...
            for client in dest_dict.keys():
                if self.config.client == "all" or self.config.client == client:
                    ...
                    self.remove_files_to_be_reprocessed(client)
                    # Check if face, skeleton and sound directories exist, create if not
                    self.create_feature_directories(client)
                    ...
                    for raw_file in raw_files:
                        ...
                        sound_annotations = self.get_sound_annotations()
    ...
    def remove_files_to_be_reprocessed(self, client):
        if self.config.force_postprocess:
            if self.config.rec_id == "all":
                # Remove skeleton, sound and face direcories
                for directory_name in ["face", "skeleton", "sound"]:
                    directory = os.path.join(self.config.destination_directory, client, directory_name)
                    if os.path.exists(directory) and directory_name in self.config.postprocess:
                        self.logger.info(f"Deleting  {directory}")
                        os.system("rm -r " + directory)
                #time.sleep(1)
            else:
                # Remove only data directories
                for directory_name in ["face", "skeleton", "sound"]:
                    directory = os.path.join(self.config.destination_directory, client, directory_name, self.config.rec_id)
                    if os.path.exists(directory) and directory_name in self.config.postprocess:
                        self.logger.info(f"Deleting  {directory}")
                        os.system("rm -r " + directory)

    def process_face(self, client, file_id, dest_dict):
        self.logger.info(f"Processing face for {client}, file id {file_id}")
        if not file_id in dest_dict[client]["face"]:
            face_dir = join(self.config.destination_directory, client, "face", file_id)
            self.logger.info(f"Creating: , {face_dir}")
            if not os.path.exists(face_dir):
                os.mkdir(face_dir)

            with open(join(self.config.destination_directory, client, "raw", file_id + ".json"), "r") as fp:
            ...
            while cap.isOpened():
                ...
                if person_bbox:
                    ...
                    face_landmarks, valid, _ = self.face_feature.get(frame[y1:y2, x1:x2].copy())
    ...
    def process_skeleton(self, client, file_id, dest_dict):
        self.logger.info(f"Processing skeleton for {client}, file id {file_id}")
        # Generate face and skeleton frames
        if not file_id in dest_dict[client]["skeleton"]:
            skeleton_dir = join(self.config.destination_directory, client, "skeleton", file_id)
            self.logger.info(f"Creating: , {skeleton_dir}")
            ...
            while cap.isOpened():
                ...
                if person_bbox:
                    x1, y1, x2, y2 = person_bbox
                    pose_keypoints, valid, _ = self.pose_feature.get(frame[y1:y2, x1:x2].copy())
    ...
    def get_sound_annotations(self):
        sound_annotations = {}
        base = self.config.destination_directory
        clients = [f for f in listdir(base) if not isfile(join(base, f))]
    ...
    def process_sound(self, client, file_id, dest_dict, sound_annotations):
        self.logger.info(f"Processing sound for {client}, file id {file_id}")
        if not file_id in dest_dict[client]["sound"]:
            sound_dir = join(self.config.destination_directory, client, "sound", file_id)
            os.mkdir(sound_dir)
            # Also create client, caretaker and environmental sub directories
            for dir in ["client", "caretaker", "environment"]:
                sub_dir = os.path.join(self.config.destination_directory, client, "sound",
                                       file_id, dir)
                os.mkdir(sub_dir)

            # Save features for the whole recording in the sound directory
            sound_file = self.config.destination_directory + client + "/raw/" + file_id + ".wav"
