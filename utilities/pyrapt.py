...
from utilities.raptparams import Raptparams
from utilities.nccfparams import Nccfparams

def pitch(audio_sample, sample_rate):
    audio = audio_sample * 16777216.0  # 16777216.0 32768.0
    audio = audio.astype(int)
    freq_estimates = numpy.array(())
    #for start_sample in range(0, len(audio), 8192):
    #    end_sample = min(start_sample + 8192, len(audio) - 1)
    #    samples = audio[start_sample:end_sample]
    #    pitch_freq = rapt((sample_rate, samples))
    #    freq_estimates = numpy.append(freq_estimates, pitch_freq)
    pitch_freq = rapt((sample_rate, audio))
    #print(f"Buffer {audio_sample[25:30]}")
    #print(f"Pitch {pitch_freq}")
    freq_estimates = numpy.append(freq_estimates, pitch_freq)
    """
        for i in range(len(freq_estimates)):
            if freq_estimates[i] > 480:
                freq_estimates[i] = 0.0
            if i > 0 and i < len(freq_estimates)-2:
                if freq_estimates[i-1] == 0.0 and freq_estimates[i+1] == 0.0:
                    freq_estimates[i] = 0.0
    """
    return freq_estimates

def rapt(original_audio, **kwargs):
    """
...
def _get_audio_data(wavfile_path):
    ...
    audio_sample, sample_rate  = sf.read(wavfile_path)
    audio_sample = audio_sample[0:150000,:]
    audio_sample *= 32768.0 #16777216.0
...
def _downsample_audio(original_audio, downsampling_rate):
    ...
    number_of_samples = int(len(original_audio[1]) * sample_rate_ratio)
